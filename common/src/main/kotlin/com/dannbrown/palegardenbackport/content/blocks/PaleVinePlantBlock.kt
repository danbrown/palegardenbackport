...
class PaleVinePlantBlock(private val vineBlock: Supplier<out GrowingPlantHeadBlock>, props: Properties) :
  WeepingVinesPlantBlock(props) {
  ...
  override fun canSurvive(pState: BlockState, pLevel: LevelReader, pPos: BlockPos): Boolean {
    ...
    return if (!this.canAttachTo(blockState)) {
      ...
      blockState.`is`(this.headBlock) || blockState.`is`(this.bodyBlock) || blockState.isFaceSturdy(
        pLevel,
        blockPos,
