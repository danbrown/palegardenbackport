...
class PaleOakHeartTrunkPlacer(baseHeight: Int, heightRandA: Int, heightRandB: Int) :
  DarkOakTrunkPlacer(baseHeight, heightRandA, heightRandB) {
  companion object {
  ...
  override fun placeTrunk(
    ...
    for (currentHeight in 0 until height) {
      ...
      if (level.isStateAtPosition(
          trunkPos,
          { state: BlockState -> state.isAir || state.canBeReplaced() || state.`is`(BlockTags.LEAVES) })
      ) {
        ...
        val logBlock =
          if (isSecondToLastLayer && randomLogIndex == 0) CreakingHeartBlock.getNaturalState() else config.trunkProvider.getState(
            random,
            trunkPos
          )
        ...
        val logBlockEast =
          if (isSecondToLastLayer && randomLogIndex == 1) CreakingHeartBlock.getNaturalState() else config.trunkProvider.getState(
            random,
            trunkPos.east()
          )
        ...
        val logBlockSouth =
          if (isSecondToLastLayer && randomLogIndex == 2) CreakingHeartBlock.getNaturalState() else config.trunkProvider.getState(
            random,
            trunkPos.south()
          )
        blockConsumer.accept(trunkPos.south(), logBlockSouth)

        val logBlockDiagonal =
          if (isSecondToLastLayer && randomLogIndex == 3) CreakingHeartBlock.getNaturalState() else config.trunkProvider.getState(
            random,
            trunkPos.east().south()
          )
        ...
        if ((offsetX < 0 || offsetX > 1 || offsetZ < 0 || offsetZ > 1) && random.nextInt(3) <= 0) {
          ...
          for (branchStep in 0 until branchHeight) {
            placeLog(
              ...
              blockConsumer,
              random,
              BlockPos(startX + offsetX, topY - branchStep - 1, startZ + offsetZ),
              config
            )
          }
          foliageAttachments.add(
            FoliagePlacer.FoliageAttachment(
              BlockPos(currentX + offsetX, topY, currentZ + offsetZ),
              0,
              false
            )
          )
  ...
  override fun type(): TrunkPlacerType<*> {
    return ModPlacerTypes.PALE_OAK_HEART_TRUNK_PLACER.get()
  }
}
