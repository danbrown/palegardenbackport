...
//      this.checkEyeBlink() // TODO: Implement eye blink
  ...
  fun getNearestPlayers(): List<Player> {
    ...
    if (fromBrain.isEmpty()) {
      ...
      val players =
        entities.filterIsInstance<Player>().filter { it.isAlive && !it.isSpectator && !it.isCreative }.toList()
  ...
  fun tearDown() {
    ...
    if (level is ServerLevel) {
      ...
      level.sendParticles(
        BlockParticleOption(
          ParticleTypes.BLOCK,
          ModBlocks.PALE_OAK.blockFamily.blocks[BlockFamily.Type.WOOD]!!.get().defaultBlockState()
        ), vec3.x, vec3.y, vec3.z, 100, d0, d1, d2, 0.0
      )
      level.sendParticles(
        BlockParticleOption(
          ParticleTypes.BLOCK,
          ModBlocks.CREAKING_HEART.get().defaultBlockState().setValue(CreakingHeartBlock.ACTIVE, true)
        ), vec3.x, vec3.y, vec3.z, 10, d0, d1, d2, 0.0
      )
    }
  ...
  companion object {
    val ATTACKING: EntityDataAccessor<Boolean> =
      SynchedEntityData.defineId(CreakingEntity::class.java, EntityDataSerializers.BOOLEAN)
