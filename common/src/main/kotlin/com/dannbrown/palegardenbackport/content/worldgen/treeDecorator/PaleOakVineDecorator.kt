...
class PaleOakVineDecorator(
  ...
  override fun type(): TreeDecoratorType<*> {
    return ModPlacerTypes.VINE_DECORATOR.get()
  }

  companion object {
    val CODEC: Codec<PaleOakVineDecorator> =
      RecordCodecBuilder.create { p_225996_: RecordCodecBuilder.Instance<PaleOakVineDecorator> ->
        p_225996_.group(
        ...
        ).apply(p_225996_) { p_225988_: Float,
                             p_225989_: Int,
                             p_225990_: Int,
                             p_225991_: BlockStateProvider,
                             p_225992_: Int,
  ...
  override fun place(pContext: Context) {
    ...
    for (blockpos in Util.shuffledCopy(pContext.leaves(), randomsource)) {
      ...
      if (!set.contains(blockpos1) && randomsource.nextFloat() < this.probability && this.hasRequiredEmptyBlocks(
          pContext,
          blockpos,
          direction
        )
        ...
        pContext.setBlock(
          tipPos,
          tipBlockProvider.getState(randomsource, tipPos)
        ) // Use the new provider for the tip block
      }
    }
  }
