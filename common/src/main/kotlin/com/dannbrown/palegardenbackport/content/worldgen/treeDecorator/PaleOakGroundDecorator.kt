...
class PaleOakGroundDecorator(
  ...
  override fun type(): TreeDecoratorType<*> {
    return ModPlacerTypes.GROUND_DECORATOR.get()
  }
  ...
  private fun placeBlockAtSecondaryProvider(pContext: Context, pPos: BlockPos) {
    ...
    if (pContext.level()
        .isStateAtPosition(blockPosBelow) { it.`is`(Blocks.DIRT) || it.`is`(Blocks.GRASS_BLOCK) } && pContext.isAir(
        blockPosAbove
      )
    ) {
//      pContext.setBlock(blockPosAbove, ForgeEventFactory.alterGround(pContext.level(), pContext.random(), blockPosAbove, secondaryBlockProvider.getState(pContext.random(), pPos)))
      pContext.setBlock(blockPosAbove, this.secondaryBlockProvider.getState(pContext.random(), pPos))
    }
  }
...
//        pContext.setBlock(
//          blockPos,
//          ForgeEventFactory.alterGround(
//            pContext.level(),
//            pContext.random(),
//            blockPos,
//            blockProvider.getState(pContext.random(), pPos)
//          )
//        )
        pContext.setBlock(blockPos, this.blockProvider.getState(pContext.random(), pPos))
        break
      }
